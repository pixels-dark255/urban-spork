...
def api_watchlist_detail(item_id: int, request: Request):
    """Full picture for the watchlist tap-through view: live price right now,
    the 90-day backtest (predicted vs actual for each day, and how accuracy
    trended from the earliest backtested period to the most recent), the
    refined signal weights currently in use, and every live prediction made
    since."""
    ip = get_client_ip(request)
    item = storage.get_item(ip, item_id)
    if not item:
    ...
    return {
        ...
        "latest_prediction": {
            ...
            "price_at_prediction": latest["price_at_prediction"],
            "predicted_price": latest["predicted_price"],
            "predicted_low": latest.get("predicted_low"),
            "predicted_high": latest.get("predicted_high"),
            "confidence": latest["confidence"],
        } if latest else None,
