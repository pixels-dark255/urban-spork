...
def run_backtest_and_refine(yf_symbol: str) -> dict:
    ...
    for i in range(start_idx, n):
        ...
        actual_price = round(float(df["Close"].iloc[i]), 2)
        ...
        history[-1]["correct_direction"] = (actual_direction == predicted_direction) if actual_direction != 0 else None
...
def _summarize(history: list[dict]) -> dict | None:
    ...
    def hit_rate(chunk):
        dirs = [h["correct_direction"] for h in chunk if h.get("correct_direction") is not None]
        return round(100 * sum(dirs) / len(dirs), 1) if dirs else None

    early = avg_abs_err(early_chunk)
    recent = avg_abs_err(recent_chunk)
    dir_early = hit_rate(early_chunk)
    dir_recent = hit_rate(recent_chunk)
    return {
        ...
        # NOTE: raw error % naturally stays noisy day to day - real price
        # moves have a large irreducible random component no amount of
        # weight-tuning removes. Directional hit-rate below is the metric
        # that should actually trend up as weights refine.
        "improved": (early is not None and recent is not None and recent < early),
        "directional_hit_rate_overall": hit_rate(resolved),
        "directional_hit_rate_early_period": dir_early,
        "directional_hit_rate_recent_period": dir_recent,
        "directional_improved": (dir_early is not None and dir_recent is not None and dir_recent >= dir_early),
    }
