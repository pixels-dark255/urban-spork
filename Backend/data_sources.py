...
# a real browser's TLS/HTTP fingerprint via curl_cffi is the fix yfinance's
# own maintainers recommend for cloud deployments.
try:
    from curl_cffi import requests as curl_requests
    _YF_SESSION = curl_requests.Session(impersonate="chrome")
except Exception as e:
    print(f"[warn] curl_cffi session unavailable, falling back to default session: {e}")
    _YF_SESSION = None
...
def _yf_download(symbol: str, period: str, interval: str) -> pd.DataFrame:
    ...
    even for a single ticker, depending on version/args)."""
    try:
        kwargs = dict(
        ...
        print(f"[warn] yf.download failed for {symbol} ({period}/{interval}): {e}")
        return pd.DataFrame()
...
def fetch_latest_price(yf_symbol: str) -> float | None:
    """Try intraday first (freshest), then fall back to the last daily close
